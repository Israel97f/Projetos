...
char Uart_init (const long int baldRate){
    ...
    if (generatorSpeed < 256) {
        ...
        TRISBbits.TRISB1 = 1;                                   // Cofifigura o pino RB1 como entrada digital
        TRISBbits.TRISB2 = 0;                                   // Cofifigura o pino RB2 como saída digital
        return 1;
    } else{
        return 0;
    }
...
void Uart_interrupt (){
    ...
    if (cont == 1) comand = temp;
        ...
        checksum = temp;
        if (checksum == (Uart_checksum(strcat(comand, val)))){
            int i;
